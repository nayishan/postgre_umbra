...
static bool sendFile(bbsink *sink, const char *readfilename, const char *tarfilename,
					 ...
					 ForkNumber relForkNum, unsigned segno,
...
#ifdef USE_UMBRA
				/* WAL summaries name L, not the mapped file offset P. */
		 ...
		 BlockNumber *incremental_blocks, unsigned truncation_block_length)
{
	...
	if (!noverify_checksums && RelFileNumberIsValid(relfilenumber)
		 ...
		 * backup, but page checksums stay keyed by logical block number.  INIT
		 * forks remain direct-mapped and can still be verified here.
		 */
		&& relForkNum == INIT_FORKNUM
#endif
		)
