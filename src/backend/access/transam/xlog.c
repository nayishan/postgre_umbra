...
xlog2_redo(XLogReaderState *record)
{
	...
	else if (info == XLOG2_HINT_DELTA)
	{
		...
		if (block->forknum != MAIN_FORKNUM || !block->has_shift ||
			!UmbraChunkActiveSlotIsValid(block->source_slot) ||
			!UmbraChunkActiveSlotIsValid(block->target_slot) ||
			block->source_slot == block->target_slot ||
			!BlockNumberIsValid(block->logical_nblocks) ||
			block->logical_nblocks <= block->blkno ||
