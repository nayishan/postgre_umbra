...
static bool
DecodeMdXLogRecord(XLogReaderState *state, XLogRecord *record, char **errormsg);
static bool
DecodeShdXLogRecord(XLogReaderState *state, XLogRecord *record, char **errormsg);
typedef struct decode_mgr
{
	bool	(*decode_xlogrecord)(XLogReaderState *state, XLogRecord *record, char **errormsg);

} decode_mgr;
static const decode_mgr decode_mgr_f[] = {
	{
		.decode_xlogrecord = DecodeMdXLogRecord,
	},
	{
		.decode_xlogrecord = DecodeShdXLogRecord,
	}
};
...
#define COPY_HEADER_FIELD(_dst, _size)			\
	...
	while (remaining > datatotal)
	{
		...
		else if (block_id <= XLR_MAX_BLOCK_ID)
		{
			...
			blk->has_image = ((fork_flags & BKPBLOCK_HAS_IMAGE) != 0);
			blk->has_data = ((fork_flags & BKPBLOCK_HAS_DATA) != 0);
			blk->has_shadow = ((fork_flags & BKPBLOCK_HAS_SHADOW) != 0);
			...
			else if (blk->has_shadow)
			{
				COPY_HEADER_FIELD(&blk->grelId, sizeof(int));
				COPY_HEADER_FIELD(&blk->status, sizeof(int));
			}
			else
			{
				/*
...
{
	return decode_mgr_f[SMGR_WHICH].decode_xlogrecord(state, record, errormsg);
}
/*
