...
static void MapPageLoadFailed(MapPageDesc *desc, const MapPageTag *tag,
							  uint32 hashcode);
				  ...
				  LWLockMode mode)
{
	...
	for (;;)
	{
		...
		PG_CATCH();
		{
			MapPageLoadFailed(desc, &tag, hashcode);
			MapPageForgetIO(slot_id);
			LWLockRelease(&desc->io_lock);
			MapPageClockFreeBuffer(slot_id);
			MapPageUnpinBuffer(slot_id);
				   ...
				   UmbraFileContext *ctx, bool conditional)
{
	 ...
	 * no output I/O state to publish: an ERROR leaves the page dirty, and lock
	 * cleanup makes it available for a later retry.
	 */
...
static void
MapPageLoadFailed(MapPageDesc *desc, const MapPageTag *tag, uint32 hashcode)
{
	uint64		state;
