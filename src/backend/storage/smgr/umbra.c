...
/*
 * Convert a relation/fork batch of captured source slots to physical blocks,
 * then sort, deduplicate, and coalesce adjacent segment-local ranges.
 */
void
									 ...
									 int nrequests)
{
	...
	for (int i = 0; i < nrequests; i++)
	{
		...
		if (!UmbraActiveSlotIsValid(requests[i].source_slot))
		{
			Assert(false);
			continue;
		}
		if (!UmbraActiveSlotPhysicalBlock(requests[i].logical_block,
										  requests[i].source_slot,
										  &physical_block))
			ereport(ERROR,
					(errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
					 errmsg("Umbra source-slot block mapping overflow for relation %u/%u/%u fork %d",
