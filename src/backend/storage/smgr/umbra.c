...
typedef struct UmbraSmgrRelationState
{
	 ...
	 *
	 * Umbra currently preserves md.c behavior, but still exercises the
	 * ownership boundary: Umbra attaches per-relation implementation state when
	 * the SMgrRelation is opened and releases it when smgr destroys the handle.
	 */
	bool		initialized;
...
umopen(SMgrRelation reln)
{
	...
	/* Initialize md.c's per-fork state before any operation that can fail. */
	mdopen(reln);
