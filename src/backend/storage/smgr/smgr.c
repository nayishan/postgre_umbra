...
/*
 * Establish storage-manager policy once the caller knows the new relation's
 * persistence.  In particular, Umbra must not infer unlogged persistence from
 * a shared relation locator.
 */
void
...
/*
 * Dispatch global storage lifecycle events to the build-selected manager.
 */
void
smgrcheckpoint(void)
{
