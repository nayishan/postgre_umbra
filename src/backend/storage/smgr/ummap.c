 ...
 * has been created.  Physical capacity is derived from each logical EOF.
 *
 * Reserved bytes are zero in version 3 and validated on reopen.  They do not
 * carry relation-generation state; a future use requires a new root format
 * version.
 */
...
ummap_root_is_valid(const UmbraMapRootData *root)
{
	...
	if (root->magic != UMMAP_ROOT_MAGIC ||
		...
		return false;
	if (!ummap_root_aux_frontier_valid(root, FSM_FORKNUM) ||
