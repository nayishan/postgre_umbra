...
#define UMMAP_ROOT_FLAG_FSM_SLOT0 0x00000002U
#define UMMAP_ROOT_FLAG_VM_SLOT0 0x00000004U
#define UMMAP_ROOT_FLAGS_KNOWN \
	(UMMAP_ROOT_FLAGS_V1 | UMMAP_ROOT_FLAG_FSM_SLOT0 | \
	 UMMAP_ROOT_FLAG_VM_SLOT0)
 ...
 *
 * Both reserved byte regions are zero in version 1 and validated on reopen.
 * They do not carry relation-generation state; a future use requires a new
 * root format version.
 */
...
static bool ummap_root_aux_slot0_active(const UmbraMapRootData *root,
									 ForkNumber forknum);
static void ummap_root_get_aux_frontiers(const UmbraMapRootData *root,
									 ForkNumber forknum,
									 BlockNumber *logical_eof,
									 BlockNumber *physical_capacity);
static void ummap_root_set_aux_frontiers(UmbraMapRootData *root,
									 ForkNumber forknum,
									 BlockNumber logical_eof,
									 BlockNumber physical_capacity);
static bool ummap_root_aux_frontiers_valid(const UmbraMapRootData *root,
										 ForkNumber forknum);
...
bool
ummap_try_validate(UmbraFileContext *ctx)
{
	char		image[UMMAP_ROOT_IMAGE_SIZE];

	/*
	 * Nonfatal probe used when recovery can defer a missing or invalid root.
	 */
	Assert(ctx != NULL);
	return ummap_root_read_valid_image(ctx, image);
}
								 ...
								 RelFileLocatorBackend rlocator)
{
	...
	Assert(ctx != NULL);
	/*
	 * Load the root cache entry and open the metadata descriptor before the
	 * truncation critical section.  The publish path later finds this entry
						 ...
						 RelFileLocatorBackend rlocator)
{
	...
	if (!ummap_root_aux_slot0_active(root, forknum))
	{
		...
		ummap_root_set_aux_frontiers(root, forknum, 0, 0);
		ummap_root_refresh_crc(root);
		entry->dirty = true;
		entry->needs_fsync = true;
		entry->wal_flush_lsn = InRecovery ? InvalidXLogRecPtr :
...
void
ummap_get_aux_frontiers(UmbraFileContext *ctx, ForkNumber forknum,
						RelFileLocatorBackend rlocator,
						BlockNumber *logical_eof,
						BlockNumber *physical_capacity)
{
	...
	{
		LWLockRelease(&entry->content_lock);
		elog(PANIC, "Umbra auxiliary frontiers require slot-0 activation");
	}
	ummap_root_get_aux_frontiers(root, forknum, logical_eof,
								  physical_capacity);
	LWLockRelease(&entry->content_lock);
}

void
ummap_set_aux_frontiers(UmbraFileContext *ctx, ForkNumber forknum,
						RelFileLocatorBackend rlocator,
						BlockNumber logical_eof,
						BlockNumber physical_capacity)
{
	...
	{
		ummap_root_set_aux_frontiers(root, forknum, logical_eof,
								  physical_capacity);
		ummap_root_refresh_crc(root);
		entry->dirty = true;
		entry->needs_fsync = true;
...
void
ummap_prepare_aux_frontiers(UmbraFileContext *ctx, ForkNumber forknum,
							RelFileLocatorBackend rlocator)
{
	UmbraMapRootEntry *entry;
	UmbraMapRootData *root;
...
void
ummap_publish_prepared_aux_frontiers(UmbraFileContext *ctx,
									 ForkNumber forknum,
									 RelFileLocatorBackend rlocator,
									 BlockNumber logical_eof,
									 BlockNumber physical_capacity)
...
ummap_validate_if_exists(UmbraFileContext *ctx, RelFileLocatorBackend rlocator)
{
	...
	Assert(ctx != NULL);
	/*
	 * A missing private root is allowed here.  When it exists, enter through
	 * the resident cache so its strict loader reports an invalid disk image as
	 * corruption.
	 */
	if (!ummap_exists(ctx))
		return;
...
/* Flush resident metadata-root cache state before copying a tablespace. */
void
...
/* Discard resident metadata-root cache state without performing I/O. */
void
...
static void
ummap_root_get_aux_frontiers(const UmbraMapRootData *root,
							 ForkNumber forknum, BlockNumber *logical_eof,
							 BlockNumber *physical_capacity)
{
	Assert(root != NULL);
	Assert(logical_eof != NULL);
	...
	{
		case FSM_FORKNUM:
			*logical_eof = root->logical_eof_fsm;
			*physical_capacity = root->physical_capacity_fsm;
			break;
		case VISIBILITYMAP_FORKNUM:
			*logical_eof = root->logical_eof_vm;
...
static void
ummap_root_set_aux_frontiers(UmbraMapRootData *root, ForkNumber forknum,
							 BlockNumber logical_eof,
							 BlockNumber physical_capacity)
{
	Assert(root != NULL);
...
static bool
ummap_root_aux_frontiers_valid(const UmbraMapRootData *root,
								 ForkNumber forknum)
{
	BlockNumber	logical_eof;
	BlockNumber	physical_capacity;
...
ummap_root_is_valid(const UmbraMapRootData *root)
{
	...
	/* Validate the decoded in-memory payload without performing I/O. */
...
{
	/*
	 * Strict callers convert a missing or invalid image into a corruption ERROR.
	 */
	if (!ummap_root_read_valid_image(ctx, image))
		ereport(ERROR,
...
ummap_root_read_valid_image(UmbraFileContext *ctx, char *image)
{
	...
	/*
	 * Nonfatal raw reader for probes and redo decisions.  Missing, short or
	 * malformed roots return false; underlying file I/O errors still propagate.
	 */
	Assert(ctx != NULL);
	Assert(image != NULL);
	if (!ummap_exists(ctx) ||
									  ...
									  UmbraFileContext *ctx, bool prepared)
{
	...
	PG_TRY();
	{
		...
		{
			/*
			 * A global cache flush has a shared root entry but no relation
			 * handle.  Its file descriptors must be backend-local.  Do not call
			 * smgropen() while holding content_lock: umopen() can validate this
			 * root through the same cache.  Use an unregistered context for this
			 * writeback and destroy it before releasing content_lock.
			 */
		...
		/*
		 * A valid root always makes MAIN mapped and its capacity authoritative.
		 * Auxiliary capacities become authoritative when their activation bits
		 * are set.  Sync every described data fork before publishing the root.
		 */
		if (prepared)
