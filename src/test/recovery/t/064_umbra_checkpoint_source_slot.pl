...
# Verify that a shift after the redo boundary but before checkpoint selection
# is captured and written to its source slot.
...
# Establish C1's clean slot-0 baseline.  The selector page is implicit here.
...
pass('C1 establishes its redo-aligned shift epoch before buffer selection');

my $marker = 'checkpoint-source-slot-marker';
...
is(read_active_slot($map_path, $block_size, $target_block), 2,
	'C1 publishes the target selector after source-slot writeback');
...
ok(index(read_physical_block($main_path, $block_size,
	...
	'C1 does not write the retained dirty page to current slot 2');
