...
# Verify MAIN-fork hint delta WAL, crash redo, and the auxiliary-FSM
# target-only hint path.
...
sub read_active_slot
{
	...
	return 0 if $nread == 0;
	$nread == 1 or BAIL_OUT("short selector read from \"$path\"");
...
# Cache replacement can persist a WAL-published selector target before a
# checkpoint. Redo restores the recorded source before applying the delta.
$node->stop('immediate');
$node->start;
...
# A consistency-checking FPI is non-applying: HINT_DELTA still materializes
...
is(scalar(@consistency_hints), 1,
	'consistency checking records one HINT_DELTA');
like($consistency_hints[0] // '', qr/\bFPW\b/,
	'consistency checking adds a non-applying FPI to HINT_DELTA');
$node->stop('immediate');
$node->start;
is(xmin_is_committed($node, 'umbra_hint_delta_consistency'), 't',
	'consistency-checked HINT_DELTA passes crash redo verification');
is($node->safe_psql(
...
SKIP:
{
	...
	# Selector cache replacement may persist the target before a checkpoint.
	# Redo reselects the WAL source before rebuilding and publishing the target.
	$node->stop('immediate');
...
is($node->safe_psql('postgres', 'SHOW full_page_writes'), 'off',
	'full_page_writes is disabled for the no-FPI hint case');
$node->safe_psql(
	'postgres', q[
CREATE TABLE umbra_hint_delta_no_fpw(id integer, payload text)
  WITH (autovacuum_enabled = false, fillfactor = 50);
INSERT INTO umbra_hint_delta_no_fpw VALUES (1, 'no fpi');
CHECKPOINT;
]);
my $no_fpw_filenode = $node->safe_psql(
	'postgres', q[SELECT pg_relation_filenode('umbra_hint_delta_no_fpw'::regclass);]);
is(xmin_is_committed($node, 'umbra_hint_delta_no_fpw'), 'f',
	'no-FPI tuple has no committed hint bit before the read');
my $no_fpw_wal_start = $node->safe_psql(
	'postgres', 'SELECT pg_current_wal_insert_lsn();');
is($node->safe_psql(
		'postgres', q[SELECT payload FROM umbra_hint_delta_no_fpw WHERE id = 1;]),
	'no fpi', 'visibility check updates the no-FPI hint');
$node->safe_psql('postgres', 'SELECT pg_switch_wal();');
my $no_fpw_wal_end = $node->safe_psql(
	'postgres', 'SELECT pg_current_wal_flush_lsn();');
my @no_fpw_wal = dump_wal(
	$node, $no_fpw_wal_start, $no_fpw_wal_end, 'no-FPI MAIN hint');
my @no_fpw_hints = grep {
	/rmgr: XLOG\b/ && /desc: FPI_FOR_HINT/ &&
	/rel \d+\/\d+\/$no_fpw_filenode fork main blk 0\b/
} @no_fpw_wal;
is(scalar(@no_fpw_hints), 1,
	'no-FPI MAIN hint emits one generic FPI_FOR_HINT record');
ok(!grep(/\bFPW\b/, @no_fpw_hints),
	'no-FPI MAIN hint has no full-page image');
ok(!grep(/slot shift: source_slot [0-2] target_slot [0-2]/, @no_fpw_hints),
	'no-FPI MAIN hint does not shift slots');
my @no_fpw_deltas = grep {
	/rmgr: XLOG2/ && /desc: HINT_DELTA/ &&
	/rel \d+\/\d+\/$no_fpw_filenode fork main blk 0\b/
} @no_fpw_wal;
is(scalar(@no_fpw_deltas), 0,
	'no-FPI MAIN hint does not emit HINT_DELTA');
