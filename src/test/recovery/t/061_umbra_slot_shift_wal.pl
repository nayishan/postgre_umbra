...
# Verify image-free active-slot shifts, their target-only WAL header, and
# redo ordering.
...
sub assert_shift_wal
{
	...
	like($selector_shifts[0] // '',
		...
		"$label: WAL decodes the target-only slot transition");
}
...
# materialize the derived source before publishing and replaying the target.
$node->stop('immediate');
$node->start;
is(read_active_slot($map_path, $block_size, $target_block), 0,
