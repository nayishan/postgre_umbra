...
use constant MAP_GENERATION_LSN_OFFSET => 32;
...
ok(!-e $old_map_file, 'normal DROP removes the old MAP generation');

$primary->safe_psql('postgres', 'CHECKPOINT');
ok(!-e $old_main_file, 'checkpoint removes the old MAIN gate');
ok(!-e $old_map_file, 'checkpoint keeps the old MAP generation removed');
$primary->wait_for_replay_catchup($standby);
is($standby->safe_psql(
		'postgres',
