...
# Verify that checkpoint capture cannot cross WAL-to-MAP publication.
...
# Start the remap after capture becomes visible.  The fresh delay snapshot in
# CheckPointGuts() must still observe it.
$node->restart;
$node->safe_psql(
	'postgres',
...
# Pause after capture is active but before the new redo pointer is visible.
# An overlapping existing-page remap must retain an APPLY image.
$node->safe_psql(
...
is(normalize_waldump_stderr($early_wal_stderr), '',
	'pg_waldump reads the early-capture update WAL');
...
# An ERROR before the redo point must not leave the shared capture epoch active.
$node->safe_psql('postgres', 'CHECKPOINT');
$node->safe_psql(
	'postgres',
	q{SELECT injection_points_attach('umbra-checkpoint-after-capture-before-redo', 'error')});
my ($failed_rc, $failed_stdout, $failed_stderr) =
  $node->psql('postgres', 'CHECKPOINT');
isnt($failed_rc, 0, 'checkpoint fails after publishing its capture epoch');
like($failed_stderr, qr/checkpoint request failed/,
	'client observes the injected checkpoint failure');
$node->safe_psql(
	'postgres',
	q{SELECT injection_points_detach('umbra-checkpoint-after-capture-before-redo')});

my $post_error_writer = $node->background_psql('postgres');
$post_error_writer->query_safe(
	'SELECT payload FROM checkpoint_publication WHERE id = 1');
my $post_error_wal_start = $node->safe_psql(
	'postgres', 'SELECT pg_current_wal_insert_lsn()');
my $post_error_block = $post_error_writer->query_safe(q{
UPDATE checkpoint_publication SET payload = repeat('e', 1000) WHERE id = 1
RETURNING (ctid::text::point)[0]::integer;
});
my $post_error_wal_end = $node->safe_psql(
	'postgres', 'SELECT pg_current_wal_insert_lsn()');
$post_error_writer->quit;

my ($post_error_wal, $post_error_wal_stderr) = run_command(
	[
		'pg_waldump', '-b', '-p', $node->data_dir . '/pg_wal',
		'--start', $post_error_wal_start, '--end', $post_error_wal_end
	]);
is(normalize_waldump_stderr($post_error_wal_stderr), '',
	'pg_waldump reads WAL after the failed checkpoint');
my @post_error_records = grep {
	/rel \d+\/\d+\/$filenode fork main blk $post_error_block/ &&
	/; remap: old_pblk \d+ new_pblk \d+ logical_nblocks \d+ next_free_pblk \d+/
} split(/\n/, $post_error_wal);
cmp_ok(scalar(@post_error_records), '>=', 1,
	'post-error update still uses existing-page remap');
ok(!grep(/\bFPW\b/, @post_error_records),
	'post-error update is not misclassified as capture-active');
...
is(
	$node->safe_psql(
		'postgres',
		q{SELECT payload = repeat('e', 1000) FROM checkpoint_publication}),
	't',
	'crash recovery preserves the update after capture cleanup');
