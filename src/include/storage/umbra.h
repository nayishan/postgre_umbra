...
/* Stored in every Umbra metadata root that uses the slot-0 formula layout. */
...
/*
 * MAIN pages are born in slot 0.  Later patches may select another slot, but
 * this patch deliberately keeps the mapping formula-only.
 */
static inline bool
